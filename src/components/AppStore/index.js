...
class AppStore extends Component {
  ...
  getactiveTabs = () => {
    ...
    const activeTabs = appsList.filter((eachapp) =>
  ...
  render() {
    ...
    const searchResult = this.getactiveTabs();
    const filterApps = this.filterAppStores(searchResult);
    return (
        ...
        <div className="app-store">
          ...
          <div className="search-input-container">
            <input
              ...
              onClick={this.onClickUserInput}
              value={searchinput}
            />
            <img src={SEARCH_ICON_URL} alt="img" className="search-icon" />
          </div>
          <ul className="tabs-list">
            {tabsList.map((eachElement) => (
              <TabItems
                key={eachElement.tabId}
                tabDetailc={eachElement}
                setActiveTab={this.setActiveTab}
                isActive={activeTabId === eachElement.tabId}
              />
            ))}
          </ul>
          <ul className="apps-list">
            {filterApps.map((eachvalue) => (
              <AppItems key={eachvalue.appId} appDetailc={eachvalue} />
            ))}
          </ul>
        </div>
      </div>
